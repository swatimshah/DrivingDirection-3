...
file = mne.io.read_raw_eeglab("Participant2_Part1&2_SelectData.set")
...
# Find all the 204 events corresponding to Left, Right and Straight 
...
for i in range (0, len(file.annotations.onset)):
	...
	if ((temp['onset'] == 203.134766 or temp['onset'] == 1001.304688)):   # Part is over when boundary and impedence events have equal "onset".
		print("Part over")
		count = 0
		continue
	if(file.annotations[i]['description'] == 's4'):
		if(diff_between_two_annot > 0.1):
			print("anotation filtered")
			count = count + 1
			print(count)
...
combined_events_and_labels = numpy.append(annot_filtered, my_labels.reshape(3, 204).transpose(), axis=1)
...
final_input_data_with_labels = numpy.append(final_input_data, combined_events_and_labels[:, 1].reshape(204, 1), axis=1)
numpy.savetxt('final_input_data_with_labels_2_800_1800.csv', final_input_data_with_labels, delimiter=',')	
