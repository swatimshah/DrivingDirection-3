...
orig_epochs = loadtxt('combined_1_2_3_4_800_1800.csv', delimiter=',', skiprows=1)
print(orig_epochs.shape)

# scale the data with robust scaler
my_epochs = rScaler.fit_transform(orig_epochs[0:1031, 0:1920])  # noise
...
# Create the model  ################## BEST ONE - SO FAR #######################
model=Sequential()
model.add(Conv1D(filters=60, kernel_size=6, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.35, max_value=0.35), padding='valid', activation='relu', strides=1, input_shape=(64, 30)))
model.add(AveragePooling1D(pool_size=2))
model.add(Conv1D(filters=60, kernel_size=6, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.35, max_value=0.35), padding='valid', activation='relu', strides=1))
model.add(GlobalMaxPooling1D())
model.add(Dense(20, kernel_regularizer=L2(0.0002), bias_regularizer=L2(0.0002), activity_regularizer = L2(0.0002), kernel_constraint=min_max_norm(min_value=-0.35, max_value=0.35), activation='relu'))
...
hist = model.fit(input, Y_train, batch_size=32, epochs=250, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None, callbacks=[es, mc])
